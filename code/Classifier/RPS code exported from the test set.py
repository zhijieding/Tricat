"""
按测试集导出的每场RPS代码 — BPCF (Bounded Per-class Calibrated Fusion)
"""
...
def calc_rps_vec(y_true, y_prob, n_class=3):
    out = np.empty(len(y_true), dtype=float)
    for i in range(len(y_true)):
        pr = np.clip(y_prob[i], 1e-12, 1.0)
        pr = pr / pr.sum()
        oh = np.zeros(n_class); oh[int(y_true[i])] = 1.0
        cp = np.cumsum(pr); co = np.cumsum(oh)
        out[i] = np.sum((cp - co) ** 2) / (n_class - 1)
    return out
...
# —— 逐样本 RPS 导出
rps_vec_b365  = calc_rps_vec(y_test_full, test_b365, 3)
rps_vec_model = calc_rps_vec(y_test_full, prob_model_test, 3)
rps_vec_blend = calc_rps_vec(y_test_full, prob_blend_test, 3)

def _safe_col(df, name, default):
    return df[name] if name in df.columns else pd.Series([default]*len(df))

rps_rows = pd.DataFrame({
    'Date'     : _safe_col(test_df, 'Date', pd.NaT),
    'HomeTeam' : _safe_col(test_df, 'HomeTeam', ''),
    'AwayTeam' : _safe_col(test_df, 'AwayTeam', ''),
    'FTR_true' : y_test_full,
    'Pred_blend': y_pred_blend,
    'RPS_B365' : rps_vec_b365,
    'RPS_Model': rps_vec_model,
    'RPS_BPCF' : rps_vec_blend,
})

rps_rows[['P_H_b365','P_D_b365','P_A_b365']] = test_b365
rps_rows[['P_H_model','P_D_model','P_A_model']] = prob_model_test
rps_rows[['P_H_BPCF','P_D_BPCF','P_A_BPCF']] = prob_blend_test

per_match_csv = os.path.join(save_dir, 'rps_per_match_test.csv')
rps_rows.to_csv(per_match_csv, index=False, encoding='utf-8-sig')
print("已导出逐样本 RPS 到：", per_match_csv)
